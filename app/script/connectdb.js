...
exports.getItemPurchaseOrderNumers=function(item_id,callback){
    connection.query('SELECT purchase_order_number FROM od_purchase_purchase_register where item_id= "'+item_id+'"',function(err,itdata){
      if(itdata.length>0){
...
connection.query("select quality_parameter_type_flag,item_id,quality_parameter_id from config_quality_parameter_type_flag_detail where item_id='"+itemid_get+"' and quality_parameter_id='"+qualityid_get+"'",function(err,rows){
// console.log(err);
   if(!err){
    //  console.log(rows);
      return callback(rows);
...
connection.query("select t1.item_id,t1.minimum_value,t1.maximum_value,t2.item_name,t3.quality_parameter_name from  od_quality_quailty_item_id_value_mapping as t1 inner join md_procurement_item_detail as t2 on (t1.item_id=t2.item_id) join md_quality_quality_parameter as t3 on t3.quality_parameter_id = t1.quality_parameter_id WHERE t1.quality_parameter_id ='"+ qualityid_get +"' AND t1.item_id ='"+ itemid_get +"'",function(err,rows){
// console.log(err);
 if(!err){
        return callback(rows);
        }
